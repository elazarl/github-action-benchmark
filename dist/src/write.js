...
async function enc(str, password) {
    const te = new TextEncoder();
    const iv = await crypto_1.webcrypto.getRandomValues(new Uint8Array(12));
...
async function loadDataJs(dataPath, password) {
    try {
...
async function storeDataJs(dataPath, data, password) {
    let plaintext = JSON.stringify(data.entries);
    let ciphertext = await enc(plaintext, password);
    const script = exports.SCRIPT_PREFIX + JSON.stringify({
...
async function writeBenchmarkToGitHubPagesWithRetry(bench, config, retry) {
    var _a, _b;
    const { name, tool, ghPagesBranch, ghRepository, benchmarkDataDirPath, githubToken, autoPush, skipFetchGhPages, maxItemsInChart, password, } = config;
...
async function loadDataJson(jsonPath, password) {
    try {
